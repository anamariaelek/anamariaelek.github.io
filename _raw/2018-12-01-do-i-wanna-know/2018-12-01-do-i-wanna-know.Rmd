...
require(plotly)
require(extrafont)
scale_breaks <- levels(df_melted$variable)
scale_labels <- c(
"s=0, f=0, l=1", 
"s=0, f=0, l=5",
"s=0, f=0, l=5, f=1",
...
gp <- ggplot(df_melted,aes(x,value)) + geom_line(aes(color=variable,linetype=variable),size=1) +
scale_color_manual(name="parameters:", breaks=scale_breaks, labels=scale_labels,values=cbPalette) +
scale_linetype_discrete(name="parameters:", breaks=scale_breaks, labels=scale_labels) + 
theme_light() + theme(
text = element_text(size=14, family="Libre Franklin"),
...
x=x, 
y0=0, 
y1=0.15*cos(1/2*x)^(0.8),
y2=-0.15*cos(1/2*x)^(0.8),
y3=0,
y4=AMfunction(x,s=0,l=0.01,L=3,g=0,f=1),
y5=0,
y6=AMfunction(x)
)
df_melted <- melt(df,id.vars="x")
...
#plot.margin=unit(c(6,0,6,0),"cm")
#aspect.ratio=1
) +
transition_states(
variable,
transition_length=1e-1,
state_length=c(rep(1e-10,4),1e-5,1e-10,1e-5)
)
animate(am, length=1e-5, width=1000, height=800)
```
